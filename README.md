...
**Cost of the tag lookup.** Resolving tags means walking every page of `/api/v1/workflows`, which on an instance with thousands of workflows is seconds. The gate prefetches it at startup — before `/readyz` reports ready — so a cold container spends its own startup budget rather than the first caller's request timeout, and the prefetch never fails the probe. After that it is refreshed on the first call past `--mcp-cache-ttl-ms`; raise the TTL if that refresh shows up in your latency.
